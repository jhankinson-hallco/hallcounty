...
- Project: Print Fax Scan (Windows Capability Win32 app plus PDQ tandem)
...
- Capability identity: resolved at runtime by name prefix `Print.Fax.Scan*`, requiring exactly one match (not a hardcoded exact revision suffix - see 2026-09-10 decision)
- Intune install: `Install-PrintFaxScan.ps1`
- Intune uninstall: `Uninstall-PrintFaxScan.ps1`
- Intune detection: `Detect.ps1`, matches by name prefix and requires `State -eq 'Installed'` on the single match
- PDQ install: `PDQ\Install-PrintFaxScan-PDQ.ps1`, followed by `PDQ\Set-PrintFaxScanMarker-PDQ.ps1`
- PDQ uninstall: `PDQ\Uninstall-PrintFaxScan-PDQ.ps1`
- Package artifact: not built; packaging remains a fully manual step (no packaging-staging utility - see 2026-09-10 decision)

## Active Risks

- Confirmed in production (2026-09-10): a real target device failed `Install-PrintFaxScan.ps1` with `Add-WindowsCapability failed. Error code = 0x800f0954` at line 142. Root cause per `Windows Update Source Diagnostics` (proven on a different Hall County endpoint, `TA-PF47WVTR`): domain-joined devices here are under a `WSUS - Internal (TC18)` GPO with `UseWUServer=1`, and WSUS deployments typically do not carry Features-on-Demand content, so `Add-WindowsCapability` cannot locate the package through the WSUS-registered source. This project's install scripts now retry via a bundled offline `.\Source\` folder (see 2026-09-10 decision below), but that folder is currently empty - a WSUS-managed device still fails until Jeremy stages the matching Features on Demand content for its Windows build. Changing the WSUS/optional-content GPO was explicitly ruled out by Jeremy as a fix path for now.
- The sibling Print Management Console app's apparent success on a target device is not proof its online path works: if the capability was already present on that device's image, the install script's idempotent "already Installed" fast path never exercises `Add-WindowsCapability` at all (Jeremy's own observation, 2026-09-10, confirmed as sound reasoning). The fix in this project applies identically to that app for the same reason.
...
- Also fixed version drift: `PDQ\Set-PrintFaxScanMarker-PDQ.ps1` still referenced its paired PDQ install script as `v1.0.1` after it had already been bumped to `v1.0.2` last turn. Corrected to `v1.0.3`.
...
- Split the PDQ marker write into `Set-PrintFaxScanMarker-PDQ.ps1`, to be run as the final PDQ install step.
- Renamed PDQ scripts to the standard `*-PDQ.ps1` form.
- Added `New-PrintFaxScanIntuneSource.ps1`, which creates a clean, versioned staging folder containing only the install, uninstall, and detection scripts and verifies their hashes.
...
- When building the `.intunewin`, copy only `Install-PrintFaxScan.ps1`, `Uninstall-PrintFaxScan.ps1`, `Detect.ps1`, and (once populated) `.\Source\` into a clean folder by hand first. Do not package the project root or the `PDQ` subfolder directly - `IntuneWinAppUtil.exe` sweeps the entire source folder it is pointed at.
...
- Files reviewed: Jeremy's real production error report (`Add-WindowsCapability failed. Error code = 0x800f0954` at line 142 of `Install-PrintFaxScan.ps1`), this project's own `Install-PrintFaxScan.ps1`/`PDQ\Install-PrintFaxScan-PDQ.ps1`, and the sibling `Windows Update Source Diagnostics` project's proven WSUS findings for Hall County domain devices.
- Files changed: `Install-PrintFaxScan.ps1`, `PDQ\Install-PrintFaxScan-PDQ.ps1` (added `Add-CapabilityWithFallback`, bumped to 1.0.3); `Detect.ps1`, `Uninstall-PrintFaxScan.ps1`, `PDQ\Uninstall-PrintFaxScan-PDQ.ps1` (version-only bump to 1.0.3, no logic change); `PDQ\Set-PrintFaxScanMarker-PDQ.ps1` (corrected stale `v1.0.1` cross-reference to `v1.0.3`, no logic change).
- Findings accepted: WSUS enforcement (proven elsewhere in this workspace) is the credible cause; Jeremy's own observation that Print Management Console's apparent prior success may just reflect the idempotent already-installed path, not a proven working download path, is sound - the fix was applied identically to both apps for that reason.
- Decision made with Jeremy: no GPO change (explicitly ruled out for now); fix via a local offline-source retry instead.
- Tests/validation performed: PS 5.1 parse (0 errors), UTF-8 BOM + ASCII-only (all 12 scripts across both projects), live re-run of the idempotent install path and `Detect.ps1` for both apps on this workstation to confirm no regression. Did NOT live-test the fallback path itself - this workstation has no WSUS restriction to reproduce against, and no destructive uninstall was performed to force the untested path.
...
- Files changed: `Detect.ps1`, `Install-PrintFaxScan.ps1`, `Uninstall-PrintFaxScan.ps1`, `PDQ\Install-PrintFaxScan-PDQ.ps1`, `PDQ\Uninstall-PrintFaxScan-PDQ.ps1` (all bumped to 1.0.2). `Set-PrintFaxScanMarker-PDQ.ps1` unchanged.
- Files removed: `New-PrintFaxScanIntuneSource.ps1`, per Jeremy's direction that it was unneeded automation.
...
- Tests/validation performed: PS 5.1 parse (0 errors), UTF-8 BOM + ASCII-only (all 12 remaining scripts), AST alias scan (none), live (non-mocked) execution of `Detect.ps1` and the idempotent "already Installed" branch of `Install-PrintFaxScan.ps1` on this workstation with real marker output inspected.
