...
- Decision: The offline-source fallback was added identically to both `Install-PrintManagementConsole.ps1`/`PDQ\Install-PrintManagementConsole-PDQ.ps1` (this project) and `Install-PrintFaxScan.ps1`/`PDQ\Install-PrintFaxScan-PDQ.ps1` (the sibling project), even though the reported production error was only on Print Fax Scan.
- Status: Accepted
...
- Decision: `New-PrintManagementConsoleIntuneSource.ps1` is deleted. This supersedes the 2026-09-09 "Intune packaging uses an isolated three-file source" decision below.
...
- Decision: `Detect.ps1` queries the exact identity `Print.Management.Console~~~~0.0.1.0`, requires exactly one result, and reports detected only when its state is Installed. Markers are not part of Intune detection.
- Status: Accepted
...
- Decision: The PDQ capability install script does not write a marker. `Set-PrintManagementConsoleMarker-PDQ.ps1` is a separate final PDQ step. This supersedes the initial 1.0.0 decision to fold marker creation into the install script.
- Status: Accepted
...
- Decision: `New-PrintManagementConsoleIntuneSource.ps1` creates a unique folder under `C:\Temp\IntunePackageSource\PrintManagementConsole` containing only the install, uninstall, and detection scripts, verifies their hashes, and does not build an `.intunewin` file.
- Status: Accepted
