...
.DESCRIPTION
    Uninstalls Centegix CrisisAlert desktop application.
...
function Get-CentegixUninstallInfo {
    <#
    .SYNOPSIS
        Finds uninstall information for Centegix from registry.
    #>
...
try {
    ...
    # Find Centegix installations
    $UninstallEntries = Get-CentegixUninstallInfo
    
    if ($UninstallEntries.Count -eq 0) {
        Write-ErrorLog -Message "Centegix detected but no uninstall entries found in registry"
        exit $EXIT_SUCCESS
    }
    ...
    if (Test-CentegixInstalled) {
        Write-ErrorLog -Message "Warning: Centegix may still be installed after uninstall attempt"
    }
