<# 
.SYNOPSIS
    Uninstall-ESDraw.ps1
    Silently uninstalls Easy Street Draw via MSI for Intune Win32 deployment.

.DESCRIPTION
    - Designed for Hybrid Join / Autopilot environments
    - Logs ONLY on failure to C:\IntuneAppLogs\ESDraw_Uninstall.txt
...
.NOTES
    ...
    Uninstall Cmd:  powershell.exe -ExecutionPolicy Bypass -File .\Uninstall-ESDraw.ps1
#>
...
# Supports partial match: script searches for entries containing this string
$DisplayName = "Easy Street Draw"
...
#   Where-Object { $_.DisplayName -like "*Easy Street Draw*" } | Select DisplayName, PSChildName
...
# Uninstall-related errors (720xx range)
$ERR_MSIEXEC_NOT_FOUND   = 72001  # msiexec.exe missing (corrupt OS)
$ERR_MSI_START_FAILED    = 72002  # Could not start msiexec process
$ERR_MSI_TIMEOUT         = 72003  # MSI uninstallation timed out
...
if (-not $ProcessCompleted) {
    ...
    Exit-WithError -ExitCode $ERR_MSI_TIMEOUT -Message "Uninstallation timed out after $TimeoutSeconds seconds. Process was terminated."
}
