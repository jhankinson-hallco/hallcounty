...
.DESCRIPTION
    Installs Lynx Notification Client using the MSI installer with organization-specific
    configuration parameters.
    
    Lynx is a mass notification and emergency communication platform that enables
    organizations to send critical alerts and messages to staff and stakeholders.
...
.NOTES
    ...
    Install Cmd:    powershell.exe -ExecutionPolicy Bypass -NoProfile -NonInteractive -File Install-LynxClient.ps1
    Uninstall Cmd:  powershell.exe -ExecutionPolicy Bypass -NoProfile -NonInteractive -File Uninstall-LynxClient.ps1
...
$MsiFileName = "LynxClient_v10.4.26.0.msi"

# ----- LYNX CLIENT CONFIGURATION -----
# These settings configure the client to connect to your organization's Lynx server

# Lynx server hostname (your organization's Lynx cloud instance)
$LynxHostname = "city-of-gainesville-cb24.stratus-lynx.com"

# Lynx client profile name
$LynxProfile = "DefaultClientProfile"

# ----- MSI PARAMETERS -----
# Standard silent install parameters plus Lynx-specific properties
$MsiParameters = "HOSTNAME=`"$LynxHostname`" PROFILE=`"$LynxProfile`" /qn /norestart"

# Installation timeout (seconds)
$InstallTimeoutSeconds = 300

# Detection paths - common installation locations for Lynx Client
$DetectionPaths = @(
...
$ERR_MSI_NOT_FOUND = 79001
$ERR_MSI_INSTALL_FAILED = 79002
$ERR_MSI_INSTALL_TIMEOUT = 79003
$ERR_DETECTION_FAILED = 79004
...
function Exit-WithError {
    ...
    Write-ErrorLog -Message "Lynx Hostname: $LynxHostname"
    Write-ErrorLog -Message "Lynx Profile: $LynxProfile"
...
# HELPER FUNCTIONS
#--------------------------------------------------------------------------------
function Test-LynxClientInstalled {
        ...
        if (Test-Path -LiteralPath $RegPath) {
            ...
            foreach ($Subkey in $Subkeys) {
                try {
                    ...
                    if ($DisplayName -like "*Lynx*Client*" -or $DisplayName -like "*Lynx Notification*") {
                        return $true
                    }
                }
                catch { }
...
try {
    ...
    # -------------------------------------------------------------------------
    # STEP 3: Install Lynx Client via MSI
    # -------------------------------------------------------------------------
    ...
    if (-not (Test-LynxClientInstalled)) {
        Exit-WithError -ExitCode $ERR_DETECTION_FAILED `
                       -Message "MSI installation completed (exit code: $MsiExitCode) but Lynx Client was not detected." `
                       -MsiExitCode $MsiExitCode
    }
