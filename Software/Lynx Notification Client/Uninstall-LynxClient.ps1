...
.DESCRIPTION
    Uninstalls Lynx Notification Client.
...
function Get-LynxClientUninstallInfo {
    <#
    .SYNOPSIS
        Finds uninstall information for Lynx Client from registry.
    #>
...
function Uninstall-MsiByProductCode {
    ...
    try {
        Write-ErrorLog -Message "Uninstalling: $DisplayName (Product: $ProductCode)"
        ...
        if (-not $ProcessCompleted) {
            ...
            Write-ErrorLog -Message "Uninstall timed out for $DisplayName"
            return $false
        }
        
        # Exit codes: 0 = success, 1605 = not installed, 3010 = reboot required
        if ($MsiProcess.ExitCode -in @(0, 1605, 3010)) {
            Write-ErrorLog -Message "Uninstall completed: $DisplayName (Exit: $($MsiProcess.ExitCode))"
            return $true
        }
        else {
            Write-ErrorLog -Message "Uninstall failed: $DisplayName (Exit: $($MsiProcess.ExitCode))"
            return $false
        }
    }
    catch {
...
try {
    ...
    # Find Lynx Client installations
    $UninstallEntries = Get-LynxClientUninstallInfo
    
    if ($UninstallEntries.Count -eq 0) {
        Write-ErrorLog -Message "Lynx Client detected but no uninstall entries found in registry"
        exit $EXIT_SUCCESS
    }
    
    # Uninstall each found entry
    foreach ($Entry in $UninstallEntries) {
        Uninstall-MsiByProductCode -ProductCode $Entry.ProductCode -DisplayName $Entry.DisplayName
    }
    ...
    if (Test-LynxClientInstalled) {
        Write-ErrorLog -Message "Warning: Lynx Client may still be installed after uninstall attempt"
    }
