...
<#
.SYNOPSIS
    Installs Microsoft Office 365 (M365 Apps for Enterprise) via the Office Deployment Tool.

.DESCRIPTION
    Deploys Microsoft Office 365 using the bundled ODT setup.exe and
    HallCounty-Default-Office-Config.xml. Intended to run after the M365 Pre-Cleanup
    Win32 app has completed, ensuring no legacy or conflicting Office installation
    remains before provisioning begins.

    - Runs setup.exe /configure with the bundled XML in System context.
    ...
    - Logs only on error to C:\IntuneAppLogs. ODT also writes its own diagnostic
      logs to C:\IntuneAppLogs via the Logging element in the XML.
...
.NOTES
    ...
    DETECTION
    Use the bundled Detect.ps1 custom detection script. Configure in Intune:
...
# ODT configuration XML file name. Must be in the same folder as this script.
...
# buffer inside the 60-min portal ceiling and well above the actual install window.
...
function Test-Configuration {
    ...
    if ([string]::IsNullOrWhiteSpace($script:ConfigXmlFileName)) {
        Exit-Failure -Code 1 -Message 'Configuration error: ConfigXmlFileName is blank.' -Category 'Intune'
    }

    if (-not (Test-IsValidLeafFileName -Value $script:ConfigXmlFileName)) {
        Exit-Failure -Code 1 -Message ('Configuration error: ConfigXmlFileName must be a valid file name only, not a path. Value: ''{0}''.' -f $script:ConfigXmlFileName) -Category 'Intune'
    }

    if ([System.IO.Path]::GetExtension($script:ConfigXmlFileName) -ine '.xml') {
        Exit-Failure -Code 1 -Message ('Configuration error: ConfigXmlFileName must point to an .xml file. Value: ''{0}''.' -f $script:ConfigXmlFileName) -Category 'Intune'
    }
    ...
    if (($script:TimeoutSeconds -lt 1) -or ($script:TimeoutSeconds -ge 3600)) {
        # Ceiling is 3599 s (just under 60 min) - matching the Win32 app portal maximum.
        ...
        Exit-Failure -Code 1 -Message ('Configuration error: TimeoutSeconds must be between 1 and 3599. Value: ''{0}''.' -f $script:TimeoutSeconds) -Category 'Intune'
    }
...
try {
    ...
    # Use %SystemRoot%\SysNative\WindowsPowerShell\v1.0\powershell.exe in the Intune portal
    # install command to guarantee 64-bit execution.
    ...
    # Validate the ODT configuration XML before launch. A missing or misnamed XML is
    # a hard dependency failure - better to exit 1 here with a clear message than to
    # have ODT fail at runtime with a less actionable error.
    $configXmlPath = Join-Path -Path $scriptRoot -ChildPath $script:ConfigXmlFileName

    if (-not (Test-Path -LiteralPath $configXmlPath -PathType Leaf)) {
        Exit-Failure -Code 1 -Message ('ODT configuration XML not found at ''{0}''. Verify the package contains ''{1}'' in the same folder as the script.' -f $configXmlPath, $script:ConfigXmlFileName) -Category 'Intune'
    }
