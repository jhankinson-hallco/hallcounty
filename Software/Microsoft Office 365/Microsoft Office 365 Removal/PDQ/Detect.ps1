...
# Path must match $script:MarkerFile in Remove-Office365.ps1.
...
$script:RequiredScriptVersion = '1.5.15'

try {
    if (-not (Test-Path -LiteralPath $script:MarkerFile -PathType Leaf)) {
        exit 1
    }
    ...
    # Marker exists but version does not match - not detected at required version.
    exit 1
}
catch {
