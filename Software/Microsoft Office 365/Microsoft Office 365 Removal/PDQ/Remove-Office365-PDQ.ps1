...
param(
    ...
    # Do NOT pass this switch in the PDQ package step.
    [switch]$AppXOnly
)
...
<#
.SYNOPSIS
    PDQ deployment: removes detected M365 component variants before managed M365 deployment.

.DESCRIPTION
    PDQ Deploy installation script. Tandem counterpart of the Intune Win32 app
    Remove-Office365.ps1 (v1.5.22).
    ...
    from the local staged folder for Click-to-Run removal. MSI, AppX, DISM, and
    OneDrive cleanup run locally on the endpoint.
    ...
    Iteratively scans for and removes the following installation types in order:
      ...
      Type 1 - Click-to-Run (C2R)
               ...
               ODT writes its own diagnostic logs to the IME Logs folder.
      ...
      Type 2 - Microsoft Store / AppX
               ...
               Bundle packages are targeted first; standalone Main packages follow.
                ...
                treated as not-installed. Other AppX enumeration/removal failures
                are logged and accepted after removal attempts so protected inbox
                AppX packages cannot cascade White Glove failures.
    ...
    PROCESS PREFLIGHT:
    PDQ sets $script:KillOfficeProcesses to $false by default. This removes the
    Autopilot/White Glove process-kill behavior used by the Intune pre-cleanup
    package and avoids terminating Office, Teams, Outlook, and OneDrive sessions
    on live assigned devices. Active processes can still block removal; those
    blockers should be handled through scheduling, maintenance windows, or a
    separate explicit close/kill workflow.
    ...
    Reboot handling is left to the management channel. The marker is only written
    after a clean full-pass scan with no reboot needed. This ensures the dependent
    Office install is not started before removal is fully settled.
    ...
    time at the point of the check. If the budget is insufficient for C2R, MSI,
    OneDrive, or the global deadline, the script fails with exit 1 rather than
    launching a child process that will eventually time out silently. AppX-only
    budget shortfalls are intentionally fail-forward: the script logs the skipped
    AppX attempt and continues toward the marker so protected inbox AppX packages
    cannot cascade White Glove failures.

    Requires 64-bit PowerShell. AppX cmdlets crash a 32-bit PS host silently.
    Configure the PDQ PowerShell step to use the 64-bit host. A hard 64-bit
    process check at startup enforces this.

    TANDEM PARITY: the completion marker is identical to the Intune package:
    C:\ProgramData\Microsoft\IntuneManagementExtension\AppMarkers\M365PreCleanup.marker
    with ScriptVersion=1.5.22. Any marker, detection, cleanup-scope, or
    logging-path change must be evaluated against BOTH deployment channels.
...
.NOTES
    ...
    Change: 06/05/2026 - DISM AppX cleanup is now fail-closed: the DISM pass returns
                         ...
                         -- ver. 1.5.15
    Change: 23/06/2026 - Excluded known protected Windows inbox Office support AppX packages
                         (Microsoft.OfficePushNotificationUtility and Microsoft.OneDriveSync)
                         from AppX detection/removal. These packages can exist on fresh
                         Windows 11 images with no Microsoft 365 Apps install and must not
                         block the cleanup marker or dependent Office deployment -- ver. 1.5.16
    Change: 24/06/2026 - Added Microsoft.OutlookForWindows to NonBlockingAppXNamePatterns.
                         Present on fresh Windows 11 24H2 devices as a provisioned inbox app;
                         not a pre-existing managed Office component. Identical treatment to
                         OfficePushNotificationUtility and OneDriveSync added in v1.5.16.
                         Confirmed field failure: AppX child exiting 1 on White Glove ESP
                         when Remove-AppxProvisionedPackage targeted this inbox package on
                         a 24H2 device with no prior Office install -- ver. 1.5.17
    Change: 24/06/2026 - Removed Microsoft.OutlookForWindows from NonBlockingAppXNamePatterns
                         so New Outlook remains an AppX removal target. Moved script and ODT
                         logs to the IME Logs folder, moved the completion marker to the IME
                         AppMarkers folder, and synced Detect.ps1/Uninstall.ps1 to use current
                         marker path with legacy fallback/removal -- ver. 1.5.18
    Change: 25/06/2026 - Introduced $script:BestEffortRemovalPatterns for AppX packages that
                         are actively targeted for removal but must not block the cleanup
                         marker if all removal attempts fail. Microsoft.OutlookForWindows is
                         placed in this category: it ships as a protected provisioned inbox
                         app on Windows 11 24H2 and cannot be removed via the AppX API
                         (E_ACCESSDENIED) or DISM in SYSTEM context on clean WG images.
                         The script still attempts both passes; if both fail, the remaining
                         best-effort packages are logged and the run continues. Invoke-
                         RemoveAppXOffice now returns a PSCustomObject (HasNonBlockingProvi-
                         sioned, HasBestEffortInstalledFailed) instead of a single boolean.
                         AppXOnly child adds exit code 3 (installed best-effort copies remain,
                         no DISM pass needed). Main loop adds $appxBestEffortDone flag to
                         prevent AppX re-entry after all attempts are exhausted, avoiding
                         MaxIterations spin on unremovable packages -- ver. 1.5.19
    Change: 25/06/2026 - Emergency WG reliability hardening: AppX cleanup is now fully
                         fail-forward. The script still attempts provisioned and installed
                         AppX removals, still attempts DISM for remaining provisioned
                         packages, and still logs all leftovers, but AppX remnants or AppX
                         child failures no longer block the completion marker. C2R, MSI,
                         OneDrive, package-file, runtime-budget, and marker-write failures
                         remain blocking -- ver. 1.5.20
    Change: 15/07/2026 - Initial PDQ release derived from v1.5.15 source. C2R payload
                         staged from the shared filestore repository; logs tagged [PDQ];
                         KillOfficeProcesses default changed to $false -- ver. 1.0.0
    Change: 15/07/2026 - Added local administrator preflight; replaced stale Intune
                         reboot wording -- ver. 1.0.1
    Change: 15/07/2026 - Fixed Exit-RebootRequired docstring: PDQ does not auto-rerun
                         after reboot; redeploy until a clean pass writes the marker -- ver. 1.0.2
    Change: 15/07/2026 - Logs best-effort payload-stage cleanup warnings -- ver. 1.0.3
    Change: 15/07/2026 - Re-derived from the RECOVERED Intune v1.5.20 source (extracted
                         from the 2026-06-25 .intunewin after the OneDrive sync
                         regression): marker now written to the IME AppMarkers path with
                         ScriptVersion=1.5.20; logs to IME Logs
                         APP_M365PreCleanup_Install.txt; AppX cleanup is fail-forward per
                         v1.5.20. All prior PDQ conversion behaviors reapplied -- ver. 1.1.0
    Change: 15/07/2026 - Synced with Intune Remove-Office365.ps1 v1.5.21:
                         DISM AppX insufficient-budget handling now logs and
                         continues as part of AppX fail-forward behavior. Marker
                         parity updated to ScriptVersion=1.5.21 -- ver. 1.1.1
    Change: 15/07/2026 - Synced with Intune Remove-Office365.ps1 v1.5.22:
                         AppX phase insufficient-budget handling now logs and
                         continues as part of AppX fail-forward behavior. Marker
                         parity updated to ScriptVersion=1.5.22 -- ver. 1.1.2

    PDQ CONFIGURATION
      Package step:  PowerShell step running Remove-Office365-PDQ.ps1
    ...
    REPOSITORY (PDQ scripts only; Intune scripts must never touch the filestore)
      Payload: \\hallcounty\filestore\mis\CDS\Intune Management Applications\Microsoft Office 365\Microsoft Office 365 Removal

    DETECTION
      The Intune counterpart uses Detect.ps1 (v1.5.22) and validates
      C:\ProgramData\Microsoft\IntuneManagementExtension\AppMarkers\M365PreCleanup.marker
      containing ScriptVersion=1.5.22 (with legacy-path fallback). PDQ writes the same
      marker so Intune cannot tell which channel completed cleanup.

    RUNTIME BUDGET NOTE
    Global deadline is 3000 s (50 min), leaving a 10-min buffer against a 60-min package timeout.
    ...
    AppX timeout is 180 s (3 min) per child-process invocation. The AppX budget guard
    logs and continues when less than 240 s remains because AppX cleanup is fail-forward.
...
$script:LogFileName = 'APP_M365PreCleanup_Install.txt'

# Marker file written after a clean full-pass scan with no reboot needed.
# NOT written when exiting 3010.
$script:MarkerFile = 'C:\ProgramData\Microsoft\IntuneManagementExtension\AppMarkers\M365PreCleanup.marker'

# ODT setup.exe file name. PDQ stages this file from RepositoryRoot before execution.
$script:SetupExeName = 'setup.exe'

# ODT removal XML file name. PDQ stages this file from RepositoryRoot before execution.
$script:RemoveXmlName = 'Remove-C2R-All.xml'

# Shared deployment repository on the filestore (PDQ scripts only).
# PDQ copies only the payload files below to a local stage before ODT execution.
$script:RepositoryRoot = '\\hallcounty\filestore\mis\CDS\Intune Management Applications\Microsoft Office 365\Microsoft Office 365 Removal'
$script:StageRoot      = 'C:\ProgramData\Microsoft\IntuneManagementExtension\ScriptFiles\M365PreCleanupPDQ'
...
# on each management-triggered re-run after reboot.
...
# 3000 s (50 min) leaves a 10-min buffer against a 60-min package timeout.
...
# PDQ default is $false to avoid Autopilot/White Glove-style forced process
# termination on live assigned devices where users may have unsaved work.
# Active processes can delay or block removal; handle those blockers through
# scheduling, maintenance windows, or a separate explicit close/kill workflow.
$script:KillOfficeProcesses = $false
...
# Known protected/inbox AppX packages that can be present on fresh Windows 11
# devices with no Microsoft 365 Apps install. They are not blockers for the
# managed Office install and should not force this prerequisite cleanup to fail.
$script:NonBlockingAppXNamePatterns = @(
    '^Microsoft\.OfficePushNotificationUtility$',
    '^Microsoft\.OneDriveSync$'
)

# AppX packages targeted for removal on a best-effort basis. These match
# $script:AppXPatterns and the script actively attempts to remove them via both
# the AppX API and DISM. If all removal attempts fail for any reason, the
# failure is logged and the script continues without blocking the cleanup marker.
# Use for packages that are desirable to remove but whose presence does not
# prevent the downstream managed Office install from succeeding.
#
# Microsoft.OutlookForWindows ships as a protected provisioned inbox app on
# Windows 11 24H2. On fresh WG images with no prior user session, the AppX API
# returns E_ACCESSDENIED and DISM also fails to remove it. Blocking the entire
# Office deployment chain on this failure is worse than accepting the app stays
# until a user logs in and uninstalls it or the OS is updated.
$script:BestEffortRemovalPatterns = @(
    '^Microsoft\.OutlookForWindows$'
)
...
function Exit-RebootRequired {
    <#
        ...
        Does NOT write the completion marker. PDQ reports success code 3010;
        reboot the device and redeploy this package until a clean full-pass
        scan writes the marker. Unlike Intune, PDQ does not automatically
        re-run the package after the reboot.
    #>
    Remove-PDQPayloadStage
    exit 3010
}
...
function Test-NonBlockingAppXPackageName {
    # Simple function - no [Parameter()] attributes. See Invoke-NativeProcess for rationale.
    param(
        [string]$Name
    )
    ...
    foreach ($pattern in $script:NonBlockingAppXNamePatterns) {
...
function Test-BestEffortRemovalPackageName {
    # Returns $true if the given package Name matches any best-effort removal pattern.
    # Best-effort packages are actively targeted but do not block on failure.
    param(
        [string]$Name
    )
    ...
    foreach ($pattern in $script:BestEffortRemovalPatterns) {
        if ($Name -match $pattern) {
            return $true
        }
    }
...
function Invoke-RemoveC2R {
    ...
    if ($script:ODTSuccessExitCodes -notcontains $exitCode) {
        throw ('ODT removal exited with unexpected code {0}. Review ODT logs in {1}.' -f $exitCode, $script:LogRoot)
    }
...
function Get-AppXOfficePackages {
    ...
    try {
        ...
        foreach ($pkg in $bundlePackages) {
            if (Test-NonBlockingAppXPackageName -Name $pkg.Name) { continue }
...
function Get-AppXOfficeProvisionedPackages {
    ...
    try {
        ...
        foreach ($pkg in $provisioned) {
            ...
            if (Test-NonBlockingAppXPackageName -Name $namePrefix) { continue }
            foreach ($pattern in $script:AppXPatterns) {
...
function Test-AppXOfficeInstalled {
    ...
    try {
        ...
        $summary = Get-ExceptionSummary -ErrorRecord $_
        Write-ErrorLog -Message ('AppX detection failed and will not block this cleanup run: {0}' -f $summary) -Category 'App'
        return $false
    }
}
...
function Invoke-RemoveAppXOffice {
    <#
        ...
        Returns a PSCustomObject with two fields:
          HasNonBlockingProvisioned    - $true when any provisioned packages remain
              after the AppX API pass. The AppXOnly child exits 2 when this is
              $true, signalling the parent to attempt a DISM removal pass as a
              second chance.
          HasBestEffortInstalledFailed - $true when installed AppX packages remain
              but no provisioned packages remain. The AppXOnly child exits 3 when
              this is $true; the parent prevents AppX re-entry in later iterations.

        AppX cleanup is intentionally fail-forward for White Glove reliability.
        The script attempts and logs removal work, but AppX leftovers do not block
    ...
    $provisionedRemovalFailed = New-Object 'System.Collections.Generic.HashSet[string]'
    $installedRemovalFailed   = New-Object 'System.Collections.Generic.HashSet[string]'

    $provisioned = Get-AppXOfficeProvisionedPackages

    foreach ($pkg in $provisioned) {
        $namePrefix   = ($pkg.PackageName -split '_')[0]
        $isBestEffort = Test-BestEffortRemovalPackageName -Name $namePrefix
        try {
            ...
            if ($hResult -eq -2147024891) {   # 0x80070005 = E_ACCESSDENIED (signed Int32)
                Write-ErrorLog -Message ('AppX provisioned package ''{0}'' removal failed - Access Denied. Continuing without blocking: {1}' -f $pkg.PackageName, $summary) -Category 'App'
            }
            elseif ($isBestEffort) {
                Write-ErrorLog -Message ('AppX provisioned package ''{0}'' best-effort removal failed (HRESULT {1}). Continuing without blocking: {2}' -f $pkg.PackageName, $hResult, $summary) -Category 'App'
            }
            else {
                Write-ErrorLog -Message ('AppX provisioned package ''{0}'' removal failed (HRESULT {1}). Continuing without blocking: {2}' -f $pkg.PackageName, $hResult, $summary) -Category 'App'
            }
            [void]$provisionedRemovalFailed.Add($pkg.PackageName)
        }
    }
    ...
    foreach ($pkg in $packages) {
        $isBestEffort = Test-BestEffortRemovalPackageName -Name $pkg.Name
        try {
            Remove-AppxPackage -Package $pkg.PackageFullName -AllUsers -ErrorAction Stop
        }
        catch {
            $hResult = [int]$_.Exception.HResult
            if ($hResult -eq -2147009295) {   # 0x80073CF1 = ERROR_PACKAGE_NOT_FOUND (signed Int32)
                # Race condition - already removed. No action needed.
            }
            elseif ($isBestEffort) {
                $summary = Get-ExceptionSummary -ErrorRecord $_
                Write-ErrorLog -Message ('AppX installed package ''{0}'' best-effort removal failed (HRESULT {1}). Continuing without blocking: {2}' -f $pkg.PackageFullName, $hResult, $summary) -Category 'App'
                [void]$installedRemovalFailed.Add($pkg.Name)
            }
            else {
                $summary = Get-ExceptionSummary -ErrorRecord $_
                Write-ErrorLog -Message ('AppX installed package ''{0}'' removal failed (HRESULT {1}). Continuing without blocking: {2}' -f $pkg.PackageFullName, $hResult, $summary) -Category 'App'
                [void]$installedRemovalFailed.Add($pkg.Name)
            }
        }
    }
    ...
    if ($remainingInstalled.Count -gt 0) {
        $installedNames = ($remainingInstalled | ForEach-Object { $_.Name }) -join ', '
        Write-ErrorLog -Message ('AppX API pass completed but installed AppX packages remain. Continuing without blocking: {0}' -f $installedNames) -Category 'App'
    }

    if ($remainingProvisioned.Count -gt 0) {
        $provisionedNames = ($remainingProvisioned | ForEach-Object { ($_.PackageName -split '_')[0] }) -join ', '
        Write-ErrorLog -Message ('AppX API pass completed but provisioned AppX packages remain. DISM will be attempted if time permits: {0}' -f $provisionedNames) -Category 'App'
    }

    return [PSCustomObject]@{
        HasNonBlockingProvisioned    = ($remainingProvisioned.Count -gt 0 -or $provisionedRemovalFailed.Count -gt 0)
        HasBestEffortInstalledFailed = ($remainingInstalled.Count -gt 0 -or $installedRemovalFailed.Count -gt 0)
    }
}
...
function Invoke-DismRemoveProtectedProvisionedPackages {
    <#
        ...
        provisioned packages. The parent logs the result and continues because
        AppX cleanup is fail-forward in v1.5.20+.
...
try {
    ...
    try {
        ...
        # Write to stderr so the error is visible in PDQ output capture,
        # then exit 1 so PDQ reports failure.
    ...
    # Configure the PDQ PowerShell step to use the 64-bit host. Fail fast with a
    ...
    if (-not (Test-IsAdministrator)) {
        Exit-Failure -Code 1 -Message 'This cleanup script requires an elevated administrative token. In PDQ, run as Deploy User or Local System with local administrator rights.' -Category 'Permissions'
    }
    ...
    #   1 - AppX child failure; parent logs and continues without blocking
    #   2 - provisioned packages remain (access-denied or best-effort failure);
    #       parent attempts DISM as a second removal pass
    #   3 - installed AppX packages remain with no provisioned packages; parent
    #       sets $appxBestEffortDone and moves on without DISM
    if ($AppXOnly) {
        try {
            $appxResult = Invoke-RemoveAppXOffice
            if ($appxResult.HasNonBlockingProvisioned) {
                exit 2
            }
            if ($appxResult.HasBestEffortInstalledFailed) {
                exit 3
            }
            exit 0
        }
    ...
    if ([string]::IsNullOrWhiteSpace($PSCommandPath)) {
        Exit-Failure -Code 1 -Message 'Cannot resolve $PSCommandPath. AppX child process cannot be spawned. Run as a saved .ps1 file from disk.' -Category 'PDQ'
    }

    $payloadRoot = Copy-PDQPayloadToLocalStage
    $setupExePath  = Join-Path -Path $payloadRoot -ChildPath $script:SetupExeName
    $removeXmlPath = Join-Path -Path $payloadRoot -ChildPath $script:RemoveXmlName
    ...
    $iteration = 0
    # Set to $true when best-effort AppX packages cannot be removed after all attempts
    # (AppX API + DISM). Prevents re-entry into AppX detection on subsequent loop
    # iterations so the loop does not exhaust MaxIterations retrying work that has
    # already been attempted and logged.
    $appxBestEffortDone = $false
    ...
    while ($true) {
        ...
        # Child exit 2: provisioned packages remain; parent attempts DISM as a
        #   second pass. Any leftovers after DISM are logged and accepted.
        # Child exit 3: installed AppX packages remain; log and accept.
        # Child failures/timeouts are logged and accepted. This protects White
        #   Glove from protected inbox AppX drift while still attempting cleanup.
        # $appxBestEffortDone short-circuits Test-AppXOfficeInstalled via -and, so the
        # full package enumeration is skipped once we have given up on best-effort work.
        if (-not $appxBestEffortDone -and (Test-AppXOfficeInstalled)) {
            ...
            $elapsedSeconds   = [int]([datetime]::UtcNow - $startTime).TotalSeconds
            $remainingSeconds = $script:RuntimeDeadlineSeconds - $elapsedSeconds
            $appxRequired     = $script:AppXTimeoutSeconds + $script:BudgetBufferSeconds
            if ($remainingSeconds -lt $appxRequired) {
                # AppX budget shortfalls are fail-forward scope (same decision as the
                # v1.5.21 DISM budget fix): log and skip AppX cleanup instead of
                # exiting, so a late AppX-only shortfall cannot block the remaining
                # MSI/OneDrive work or the completion marker.
                Write-ErrorLog -Message ('Iteration {0} - Insufficient runtime budget to attempt AppX cleanup. Required: {1} s (estimated {2} s + {3} s buffer). Remaining: {4} s. Elapsed: {5} s. Continuing without blocking AppX cleanup.' -f $iteration, $appxRequired, $script:AppXTimeoutSeconds, $script:BudgetBufferSeconds, $remainingSeconds, $elapsedSeconds) -Category 'App'
                $appxBestEffortDone = $true
                continue
            }
            ...
            try {
                ...
                $summary = Get-ExceptionSummary -ErrorRecord $_
                Write-ErrorLog -Message ('Iteration {0} - AppX child process failed. Continuing without blocking: {1}' -f $iteration, $summary) -Category 'App'
                $appxBestEffortDone = $true
            }

            if ($appxBestEffortDone) {
                # AppX already logged and was accepted as best-effort.
            }
            elseif ($appxExitCode -eq 2) {
                # Provisioned packages remain. Attempt DISM as a second pass.
                Write-ErrorLog -Message ('Iteration {0} - AppX child exited 2: provisioned packages remain. Attempting DISM removal pass.' -f $iteration) -Category 'App'

                $dismRemainingProvisioned = $null
                try {
                    $dismRemainingProvisioned = @(Get-AppXOfficeProvisionedPackages)
                }
                catch {
                    $summary = Get-ExceptionSummary -ErrorRecord $_
                    Write-ErrorLog -Message ('Iteration {0} - DISM package query failed. Continuing without blocking AppX cleanup: {1}' -f $iteration, $summary) -Category 'App'
                    $appxBestEffortDone = $true
                }

                if (-not $appxBestEffortDone -and $dismRemainingProvisioned.Count -gt 0) {
                    $elapsedSeconds   = [int]([datetime]::UtcNow - $startTime).TotalSeconds
                    $remainingSeconds = $script:RuntimeDeadlineSeconds - $elapsedSeconds
                    $dismEstimate     = $dismRemainingProvisioned.Count * $script:DismTimeoutSeconds
                    $dismRequired     = $dismEstimate + $script:BudgetBufferSeconds
                    if ($remainingSeconds -lt $dismRequired) {
                        Write-ErrorLog -Message ('Iteration {0} - Insufficient runtime budget to attempt DISM AppX removal. Required: {1} s (estimated {2} s + {3} s buffer). Remaining: {4} s. Elapsed: {5} s. Continuing without blocking AppX cleanup.' -f $iteration, $dismRequired, $dismEstimate, $script:BudgetBufferSeconds, $remainingSeconds, $elapsedSeconds) -Category 'App'
                        $appxBestEffortDone = $true
                    }
                }

                if (-not $appxBestEffortDone) {
                    $dismClean = Invoke-DismRemoveProtectedProvisionedPackages
                    if (-not $dismClean) {
                        # DISM could not remove all remaining provisioned packages.
                        # Log and continue. AppX is best-effort in this package because
                        # protected inbox packages can block White Glove.
                        $postDismProvisioned = $null
                        try {
                            $postDismProvisioned = @(Get-AppXOfficeProvisionedPackages)
                        }
                        catch {
                            $summary = Get-ExceptionSummary -ErrorRecord $_
                            Write-ErrorLog -Message ('Iteration {0} - Post-DISM package query failed. Continuing without blocking: {1}' -f $iteration, $summary) -Category 'App'
                            $appxBestEffortDone = $true
                        }

                        if (-not $appxBestEffortDone) {
                            if ($postDismProvisioned.Count -gt 0) {
                                $remainingNames = ($postDismProvisioned | ForEach-Object { ($_.PackageName -split '_')[0] }) -join ', '
                                Write-ErrorLog -Message ('Iteration {0} - DISM could not remove remaining provisioned AppX packages. Continuing without blocking: {1}' -f $iteration, $remainingNames) -Category 'App'
                            }
                            else {
                                Write-ErrorLog -Message ('Iteration {0} - DISM did not report clean success, but no targeted provisioned AppX packages remain. Continuing.' -f $iteration) -Category 'App'
                            }
                            $appxBestEffortDone = $true
                        }
                    }
            ...
            elseif ($appxExitCode -eq 3) {
                # Installed copies of AppX packages remain; no provisioned
                # packages left so DISM is not applicable. Fall through to MSI/OneDrive.
                Write-ErrorLog -Message ('Iteration {0} - AppX child exited 3: installed AppX packages remain after removal attempts. Continuing cleanup without failing.' -f $iteration) -Category 'App'
                $appxBestEffortDone = $true
            }
            elseif ($appxExitCode -ne 0) {
                Write-ErrorLog -Message ('Iteration {0} - AppX child process exited {1}. Continuing without blocking. See prior error log lines for details.' -f $iteration, $appxExitCode) -Category 'App'
                $appxBestEffortDone = $true
            }
            else {
                continue
    ...
    # Marker write failure is hard exit 1 (P23 - without marker detection never succeeds).
    try {
