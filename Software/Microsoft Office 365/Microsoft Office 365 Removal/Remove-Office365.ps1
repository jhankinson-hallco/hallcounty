...
.NOTES
    ...
    Change: 15/07/2026 - Fixed the AppX DISM budget branch so an insufficient
                         DISM budget logs and continues as part of AppX fail-forward
                         handling instead of calling the blocking budget helper
                         that exits 1. Updated AppX documentation to match
                         fail-forward behavior -- ver. 1.5.21
    Change: 15/07/2026 - Extended the same fail-forward decision to the AppX phase
                         budget check: an AppX-only runtime-budget shortfall now
                         logs and skips AppX cleanup instead of calling the
                         blocking budget helper that exits 1 -- ver. 1.5.22
