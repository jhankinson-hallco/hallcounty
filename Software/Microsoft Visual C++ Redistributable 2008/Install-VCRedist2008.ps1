...
function Get-UninstallGuidSet {
    ...
    foreach ($root in $roots) {
        ...
        foreach ($k in (Get-ChildItem -Path $root -ErrorAction SilentlyContinue)) {
            $name = $k.PSChildName
            if ($name -match '^\{[0-9A-Fa-f-]{36}\}$') { [void]$set.Add($name.ToUpperInvariant()) }
        }
    }
    return $set
}
function Get-UninstallRootForGuid([string]$Guid) {
...
function Select-GuidDeterministically([string[]]$Guids) {
    $wow = @(); $std = @()
    foreach ($g in $Guids) {
        $where = Get-UninstallRootForGuid -Guid $g
        if ($where -eq 'WOW6432Node') { $wow += $g } elseif ($where -eq 'Standard') { $std += $g }
    }
...
try {
    ...
    $proc = Start-Process -FilePath $exePath -ArgumentList $InstallerArgs -Wait -PassThru -WindowStyle Hidden
    $rc = [int]$proc.ExitCode

    if ($rc -eq 1618) { exit $EXIT_RETRY }
    if ($rc -ne 0 -and $rc -ne 3010 -and $rc -ne 1641) { Write-ErrorLog "Installer returned exit code $rc."; exit $EXIT_FAILURE }

    Start-Sleep -Seconds 5
    $after = Get-UninstallGuidSet

    $new = @()
    foreach ($g in $after) { if (-not $before.Contains($g)) { $new += $g } }
