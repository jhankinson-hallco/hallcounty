<# 
.SYNOPSIS
    Install-MobileCAD-SO.ps1
    Installs Mobile CAD SO Client, configures it, and launches for the logged-in user.

.DESCRIPTION
    1. Copies "Mobile Client SO" folder to C:\Mobile Client SO
    2. Updates Configuration_SoftwareUpdate.xml (first line must contain hcso.classic)
    3. Updates Configuration_RegistrationID.xml (replaces "replaceme" with HCSO<SERIAL>)
    4. Copies SO.Classic.lnk to Public Desktop and Startup folder
    5. Launches the application in the logged-in user's context
    
    Logs ONLY on failure to C:\IntuneAppLogs\MobileCAD-SO_Install.txt
...
.NOTES
    ...
    Install Cmd:    powershell.exe -ExecutionPolicy Bypass -File .\Install-MobileCAD-SO.ps1
    Uninstall Cmd:  powershell.exe -ExecutionPolicy Bypass -File .\Uninstall-MobileCAD-SO.ps1
...
$SoftwareUpdateFirstLine = "hcso.classic"
$RegistrationIdToken = "replaceme"
$RegistrationIdPrefix = "HCSO"
...
function Update-ConfigurationFiles {
    ...
    # Update Registration ID XML - replace token with HCSO<SERIAL>
