...
# Build the arguments string conditionally.
# removebloat.ps1 -customwhitelist and -custombloatlist are [string[]] parameters.
# Passing an empty string ("") is treated as a flag with no value and throws
# "Missing an argument for parameter 'customwhitelist'". Only include each
# parameter when its value is non-empty.
$arguments = ""

if (-not [string]::IsNullOrWhiteSpace($whitelistApps)) {
    $arguments += " -customwhitelist `"$whitelistApps`""
}

if (-not [string]::IsNullOrWhiteSpace($bloatApps)) {
    $arguments += " -custombloatlist `"$bloatApps`""
}
