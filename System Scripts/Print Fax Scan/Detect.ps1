...
<#
.SYNOPSIS
    Detects the Print Fax Scan Windows Capability for Intune.
...
.NOTES
    ...
    Purpose:        Detect whether the Print Fax Scan capability is installed
...
try {
    ...
    if ($Capabilities.Count -eq 1 -and $Capabilities[0].State -eq 'Installed') {
        Write-Output 'Detected: Print Fax Scan capability state is Installed.'
        exit 0
    }
    exit 1
}
