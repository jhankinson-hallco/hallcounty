...
<#
.SYNOPSIS
    Installs the Print Fax Scan Windows Capability.

.DESCRIPTION
    Resolves the exact Print.Fax.Scan Windows Capability by name prefix
    (tolerant of a revision-suffix change across Windows builds), adds it
    when needed, validates its resulting DISM state, writes the Intune
    marker, and best-effort removes any stale PDQ marker so it does not
    survive an Intune takeover. Reboot-pending success returns 3010
...
.NOTES
    ...
    Purpose:        Install the Print Fax Scan Windows Capability
    ...
    INTUNE CONFIGURATION
      Install command:
        %SystemRoot%\SysNative\WindowsPowerShell\v1.0\powershell.exe -ExecutionPolicy Bypass -NoProfile -NonInteractive -File .\Install-PrintFaxScan.ps1
