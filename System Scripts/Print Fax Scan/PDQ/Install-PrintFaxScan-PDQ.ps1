...
<#
.SYNOPSIS
    Installs the Print Fax Scan Windows Capability through PDQ Deploy.

.DESCRIPTION
    PDQ counterpart to Install-PrintFaxScan.ps1. Resolves the exact
    capability by name prefix (tolerant of a revision-suffix change across
    Windows builds), adds it when needed, validates its resulting DISM
    state, and returns 3010 for reboot-pending success. A capability state
    of UninstallPending means a removal is in progress, not an install, so
    it is treated as an unexpected precondition rather than a completed
    install. The separate marker companion must run as the last PDQ
    package step. No filestore payload is required, so Local System is
...
.NOTES
    ...
    Purpose:        PDQ install step for the Print Fax Scan Windows Capability
    ...
    PDQ CONFIGURATION
      ...
      Final step: Set-PrintFaxScanMarker-PDQ.ps1

    INTUNE COUNTERPART
      Install-PrintFaxScan.ps1 v1.0.3
#>
...
$script:LogFile          = Join-Path -Path $script:LogRoot -ChildPath 'SCRIPT_PrintFaxScan_Install.txt'
