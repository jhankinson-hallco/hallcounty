...
<#
.SYNOPSIS
    Uninstalls the Print Fax Scan Windows Capability through PDQ Deploy.

.DESCRIPTION
    PDQ counterpart to Uninstall-PrintFaxScan.ps1. Resolves the exact
    capability by name prefix (tolerant of a revision-suffix change across
    Windows builds), removes it when present, validates its resulting DISM
    state, and best-effort removes both Intune and PDQ marker tiers,
    logging a warning rather than failing the run if a marker cannot be
    removed - the capability removal itself is what this script is
    responsible for proving. Reboot-pending success returns 3010. No
...
.NOTES
    ...
    Purpose:        PDQ uninstall step for the Print Fax Scan Windows Capability
    ...
    CHANGE LOG
    Change: 10/09/2026 - No functional change; version bumped for cross-file version-number consistency with Uninstall-PrintFaxScan.ps1 v1.0.3 -- ver. 1.0.3
    ...
    INTUNE COUNTERPART
      Uninstall-PrintFaxScan.ps1 v1.0.3
#>
