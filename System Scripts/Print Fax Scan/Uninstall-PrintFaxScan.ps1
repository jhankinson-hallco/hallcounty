...
<#
.SYNOPSIS
    Uninstalls the Print Fax Scan Windows Capability.

.DESCRIPTION
    Resolves the exact Print.Fax.Scan Windows Capability by name prefix
    (tolerant of a revision-suffix change across Windows builds), removes
    it when present, validates its resulting DISM state, and best-effort
    removes the Intune and PDQ marker tiers, logging a warning rather than
    failing the run if a marker cannot be removed - the capability removal
    itself is what this script is responsible for proving. Reboot-pending
    success returns 3010. Logs only on error.
...
.NOTES
    ...
    Purpose:        Uninstall the Print Fax Scan Windows Capability
    ...
    CHANGE LOG
    Change: 10/09/2026 - No functional change; version bumped for cross-file version-number consistency with Install-PrintFaxScan.ps1 v1.0.3 -- ver. 1.0.3
    ...
    INTUNE CONFIGURATION
      Uninstall command:
        %SystemRoot%\SysNative\WindowsPowerShell\v1.0\powershell.exe -ExecutionPolicy Bypass -NoProfile -NonInteractive -File .\Uninstall-PrintFaxScan.ps1
...
$script:AppName           = 'PrintFaxScan'
$script:AppVersion        = '1.0.2'
$script:CapabilityFilter  = 'Print.Fax.Scan*'
...
try {
    ...
    Write-ErrorLog -Message "Uninstall failed at line $($_.InvocationInfo.ScriptLineNumber): $($_.Exception.Message)"
    exit 1
}
