...
<#
.SYNOPSIS
    Uninstalls the Print Management Console Windows Capability.
...
.DESCRIPTION
    ...
    removes it when present, validates its resulting DISM state, and
    best-effort removes the Intune and PDQ marker tiers, logging a warning
    rather than failing the run if a marker cannot be removed - the
    capability removal itself is what this script is responsible for
    proving. Reboot-pending success returns 3010. Logs only on error.
...
.NOTES
    ...
    Purpose:        Uninstall the Print Management Console Windows Capability
    ...
    INTUNE CONFIGURATION
      Uninstall command:
        %SystemRoot%\SysNative\WindowsPowerShell\v1.0\powershell.exe -ExecutionPolicy Bypass -NoProfile -NonInteractive -File .\Uninstall-PrintManagementConsole.ps1
