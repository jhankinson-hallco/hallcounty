...
<#
.SYNOPSIS
    Removes the serial identification file and all marker tiers for this app.

.DESCRIPTION
    Intune Win32 app uninstall script. Runs as SYSTEM in device context.

    - Removes any "Serial - *.txt" file(s) from
      C:\ProgramData\Microsoft\IntuneManagementExtension\IntuneFiles.
    - Removes the Intune marker, PDQ marker, and legacy marker paths for this app (2026-08-20
      policy: every uninstall removes all marker tiers regardless of which
      channel wrote them), so no leftover marker can falsely satisfy a
      future detection run.
    - Attempts every cleanup tier even if one removal fails, verifies each
      removal, and returns a non-zero code if an intended artifact remains.
    - Logs only on warning/error to
      C:\ProgramData\Microsoft\IntuneManagementExtension\Logs\SCRIPT_SerialMarker_Uninstall.txt.
...
.NOTES
    ...
    INTUNE CONFIGURATION
      Uninstall command:
        %SystemRoot%\SysNative\WindowsPowerShell\v1.0\powershell.exe -ExecutionPolicy Bypass -NoProfile -NonInteractive -File .\Uninstall-SerialMarker.ps1
...
function Remove-SerialFiles {
    ...
    try {
        ...
        Write-ErrorLog -ErrorMessage "[App] Failed to enumerate serial files in '$script:TargetDir': $($_.Exception.Message)" -ErrorCode $ERR_FILE_REMOVE
        return $false
    }
    ...
    foreach ($File in $Existing) {
        try {
            ...
            $Succeeded = $false
            Write-ErrorLog -ErrorMessage "[App] Failed to remove serial file '$($File.FullName)': $($_.Exception.Message)" -ErrorCode $ERR_FILE_REMOVE
        }
    }
    ...
    try {
        ...
        if ($Remaining.Count -gt 0) {
            $Succeeded = $false
            Write-ErrorLog -ErrorMessage "[App] Serial-file cleanup verification found $($Remaining.Count) remaining file(s) in '$script:TargetDir'." -ErrorCode $ERR_FILE_REMOVE
        }
    }
    catch {
        $Succeeded = $false
        Write-ErrorLog -ErrorMessage "[App] Failed to verify serial-file cleanup in '$script:TargetDir': $($_.Exception.Message)" -ErrorCode $ERR_FILE_REMOVE
    }
...
function Remove-AllMarkerTiers {
    ...
    foreach ($MarkerPath in $MarkerPaths) {
        try {
            ...
            $Succeeded = $false
            Write-ErrorLog -ErrorMessage "[App] Failed to remove marker '$MarkerPath': $($_.Exception.Message)" -ErrorCode $ERR_MARKER_REMOVE
        }
    }
