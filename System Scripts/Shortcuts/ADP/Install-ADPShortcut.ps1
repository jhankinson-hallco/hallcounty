...
<#
.SYNOPSIS
    Deploys the ADP (Workforce Now) shortcut to the Public Desktop and sets its custom icon.
...
.DESCRIPTION
    ...
    C:\ProgramData\Microsoft\IntuneManagementExtension\Logs\SCRIPT_ADPShortcut_Install.txt.
...
.NOTES
    ...
    Purpose:        Deploy the ADP (Workforce Now) Public Desktop shortcut with custom icon
    ...
    INTUNE CONFIGURATION
      Install command:
        %SystemRoot%\SysNative\WindowsPowerShell\v1.0\powershell.exe -ExecutionPolicy Bypass -NoProfile -NonInteractive -File .\Install-ADPShortcut.ps1
      Uninstall command:
        %SystemRoot%\SysNative\WindowsPowerShell\v1.0\powershell.exe -ExecutionPolicy Bypass -NoProfile -NonInteractive -File .\Uninstall-ADPShortcut.ps1
      Install behavior: System
      Device restart behavior: No specific action
      Detection rule: custom detection script Detect-ADPShortcut.ps1
...
$script:AppName          = 'ADPShortcut'
$script:ShortcutFileName = 'ADP (Workforcenow).url'
$script:IconFileName     = 'ADP Icon.ico'
