...
<#
.SYNOPSIS
    PDQ deployment: installs the ADP (Workforce Now) shortcut to the Public Desktop with its custom icon.

.DESCRIPTION
    PDQ Deploy installation script. Tandem counterpart of the Intune Win32 app
    Install-ADPShortcut.ps1 (v1.1.1). Pulls the shortcut and icon from the shared
    ...
    Intune detection script (Detect-ADPShortcut.ps1) passes no matter which channel
...
.NOTES
    ...
    Purpose:        Deploy the ADP (Workforce Now) Public Desktop shortcut with custom icon via PDQ
    ...
    Change: 13/07/2026 - Initial PDQ release, tandem with Intune Install-ADPShortcut.ps1 v1.1.0 -- ver. 1.0.0
