...
<#
.SYNOPSIS
    PDQ deployment: removes the ADP (Workforce Now) shortcut from the Public Desktop.

.DESCRIPTION
    PDQ Deploy uninstallation script. Tandem counterpart of the Intune Win32
    uninstall Uninstall-ADPShortcut.ps1 (v1.1.1); the on-device result is identical.
    ...
    C:\ProgramData\Microsoft\IntuneManagementExtension\Logs\SCRIPT_ADPShortcut_Uninstall.txt
...
.NOTES
    ...
    Purpose:        Remove the ADP (Workforce Now) Public Desktop shortcut via PDQ
    ...
    Change: 13/07/2026 - Initial PDQ release, tandem with Intune Uninstall-ADPShortcut.ps1 v1.1.0 -- ver. 1.0.0
