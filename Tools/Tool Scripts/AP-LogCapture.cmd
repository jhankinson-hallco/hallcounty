...
.DESCRIPTION
    ...
    Output: D:\Logs\AP_<yyyyMMdd-HHmmss>\
      ...
      HallCounty\    -- IntuneAppLogs, IntuneScriptLogs, IntuneAppMarkers,
...
# App install/uninstall error logs
Copy-Tree -SourcePath 'C:\IntuneAppLogs'         -DestPath (Join-Path $HcDir 'AppLogs')

# Script-only error logs
Copy-Tree -SourcePath 'C:\IntuneScriptLogs'      -DestPath (Join-Path $HcDir 'ScriptLogs')
...
# Deployment assets (prefix file, images, configs)
Copy-Tree -SourcePath 'C:\IntuneDeploymentFiles' -DestPath (Join-Path $HcDir 'DeploymentFiles')

# End-user-facing scripts (sync buttons, etc.)
Copy-Tree -SourcePath 'C:\IntuneScripts'         -DestPath (Join-Path $HcDir 'Scripts')
